...
# LLM 配置：兼容 maching（LLM_* 优先）与 SuperMew（ARK_*/MODEL/BASE_URL）
LLM_API_KEY = os.getenv("LLM_API_KEY") or os.getenv("ARK_API_KEY") or ""
LLM_BASE_URL = os.getenv("LLM_BASE_URL") or os.getenv("BASE_URL") or "https://api.deepseek.com/v1"
LLM_MODEL = os.getenv("LLM_MODEL") or os.getenv("MODEL") or "deepseek-chat"
GRADE_MODEL = os.getenv("GRADE_MODEL") or LLM_MODEL  # 相关性评分模型，默认同主模型
...
def _get_grader_model():
    ...
    if not LLM_API_KEY or not GRADE_MODEL:
        return None
