"""文本向量化服务 - 支持密集向量（本地 bge-m3）和稀疏向量（BM25），词表与 df 持久化 + 增量更新"""
import json
import math
import os
import re
...
# 为国内网络环境配置 HuggingFace 镜像（如已设置环境变量可忽略）
...
class EmbeddingService:
    """文本向量化服务 - 密集向量（本地 bge-m3） + BM25 稀疏向量（持久化统计）"""
    ...
    def persist(self) -> None:
        """公共方法：持久化 BM25 状态到文件（由外部在批量处理完成后调用）"""
        self._persist()

    def increment_add_documents(self, texts: list[str], persist: bool = False) -> None:
        """
        将每个 text 视为 BM25 中的一篇文档（与当前 chunk 写入粒度一致），增量更新 N / df / 长度和。
        优化：默认不立即持久化，由调用方在批量处理完成后调用 persist()
        """
        if not texts:
            return
        with self._lock:
    ...
    def get_embeddings(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            ...
            raise Exception(f"embedding 接口调用失败: {str(e)}") from e
...
class _LazyEmbeddingService:
    """惰性加载 EmbeddingService，模块导入时不触发模型下载。"""
    _instance = None
...
# 使用惰性加载，导入模块时不会触发模型下载，第一次调用时才下载
