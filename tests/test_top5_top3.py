...
前置：
    ...
    3) 运行评测：            python tests/test_top5_top3.py
    （本脚本不依赖 langchain 顶层包，仅用 backend.embedding / backend.milvus_client）

⚠️ 路径说明（从项目根迁入 tests/ 时修正）：
    原实现用 `sys.path.insert(0, dirname(__file__))` 与相对路径 "eval_golden.json"，
    两者都隐含假设「脚本在项目根目录」。迁入 tests/ 后：
      - sys.path 插入的是 tests/，`from backend.xxx import` 会失败
      - .env 会去找 tests/.env（不存在），导致 RERANK_* 配置全空
      - GOLDEN_PATH / OUTPUT_PATH 依赖 CWD，换个目录执行就找不到文件
    现统一改为基于 __file__ 推导，从任意工作目录执行均可。
"""
import json
import os
import sys
...
# 关键：插入项目根而非脚本自身目录，否则 `from backend.xxx import` 找不到包
...
# 金标集与报告都放在 tests/ 下，与脚本同目录
GOLDEN_PATH = _TESTS_DIR / "eval_golden.json"
OUTPUT_PATH = _TESTS_DIR / "eval_report.json"
...
def main():
    if not GOLDEN_PATH.exists():
        print(f"✗ 找不到金标集：{GOLDEN_PATH}")
        print("  请确认 eval_golden.json 已随脚本一同迁入 tests/ 目录")
        return
